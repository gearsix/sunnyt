...
/**
 * `initDataset` & `requestData`
 * Then find print the (formatted) `dataset` console for user and find the
 * item with the earliest sunrise time.
**/
requestData(cfg, initDataset(cfg)).then((dataset: SunriseSunset[]) => {
  ...
  for (let i = 0; i < dataset.length; i++) {
    ...
    if (earliest === -1)
      earliest = i
    if (dataset[earliest].data.sunrise > d.data.sunrise)
      earliest = i
  }
  if (voidDates > 0)
